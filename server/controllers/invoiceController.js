...
exports.createInvoice = async (req, res) => {
  try {
    ...
    if (!lineItems || !Array.isArray(lineItems) || lineItems.length === 0) {
      ...
      if (amt <= 0) return res.status(400).json({ message: 'A valid total amount or lineItems array is required' });
      lineItems = [{ label: description, amount: amt }];
      totalAmount = amt;
    } else {
      totalAmount = lineItems.reduce((sum, li) => sum + Number(li.amount || 0), 0);
    }
    ...
    const invoice = await Invoice.create({ student, term, lineItems, totalAmount });
...
exports.recordPayment = async (req, res) => {
  try {
    ...
    if (req.user.role === 'student') {
      const ownsInvoice = req.user.studentProfile && invoice.student.toString() === req.user.studentProfile.toString();
      if (!ownsInvoice) return res.status(403).json({ message: 'Forbidden: not your invoice' });
    }

    await Payment.create({ invoice: invoice._id, amount, method: method || 'simulated' });
...
exports.getStudentInvoices = async (req, res) => {
  try {
    ...
    const payments = await Payment.find({ invoice: { $in: invoiceIds } }).sort({ date: -1 });
    ...
    const formatted = invoices.map((inv) => {
      ...
      obj.description = inv.lineItems?.map((li) => li.label).join(', ') || 'School Fees';
      return obj;
    });
